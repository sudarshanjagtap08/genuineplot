...
class InformationController extends Controller
{
    ...
    public function profileupdate(Request $request)
    {
        ...
        if (request()->hasFile('personal_photo')) {
            ...
            $userFolder = 'user_' . $user->name;
            $destinationPath = './images/' . $userFolder;
            if (!file_exists($destinationPath)) {
                mkdir($destinationPath, 0755, true);
            }
            $image->move($destinationPath, $filename);
            $user->photo_url = $userFolder . '/' . $filename;
        }
        $user->save();
        ...
        if (request()->hasFile('pan_card_upload')) {
            $image = $request->file('pan_card_upload');
            $filename = $image->getClientOriginalName();
            $userFolder = 'user_' . $user->name;
            $destinationPath = './images/' . $userFolder;
            if (!file_exists($destinationPath)) {
            ...
            $image->move($destinationPath, $filename);
            $buyer->pan_card_upload = $userFolder . '/' . $filename;
        }
        if (request()->hasFile('adhaar_card_upload')) {
            $image = $request->file('adhaar_card_upload');
            $filename = $image->getClientOriginalName();
            $userFolder = 'user_' . $user->name;
            $destinationPath = './images/' . $userFolder;
            if (!file_exists($destinationPath)) {
            ...
            $image->move($destinationPath, $filename);
            $buyer->adhaar_card_upload = $userFolder . '/' . $filename;
        }
        if (request()->hasFile('passport_upload')) {
            $image = $request->file('passport_upload');
            $filename = $image->getClientOriginalName();
            $userFolder = 'user_' . $user->name;
            $destinationPath = './images/' . $userFolder;
            if (!file_exists($destinationPath)) {
            ...
            $image->move($destinationPath, $filename);
            $buyer->passport_upload = $userFolder . '/' . $filename;
        }
        if (request()->hasFile('mbl_ll_bill_upload')) {
            $image = $request->file('mbl_ll_bill_upload');
            $filename = $image->getClientOriginalName();
            $userFolder = 'user_' . $user->name;
            $destinationPath = './images/' . $userFolder;
            if (!file_exists($destinationPath)) {
            ...
            $image->move($destinationPath, $filename);
            $buyer->mbl_ll_bill_upload = $userFolder . '/' . $filename;
        }
    ...
    public function profileupdateseller(Request $request)
    {
        ...
        if (request()->hasFile('pan_card_upload')) {
            ...
            $image->move($destinationPath, $filename);
            $seller->pan_card_upload = $userFolder . '/' . $filename;
        }
        if (request()->hasFile('adhaar_card_upload')) {
            $image = $request->file('adhaar_card_upload');
            $filename = $image->getClientOriginalName();
            ...
            $image->move($destinationPath, $filename);
            $seller->adhaar_card_upload = $userFolder . '/' . $filename;
        }
        if (request()->hasFile('passport_upload')) {
            $image = $request->file('passport_upload');
            $filename = $image->getClientOriginalName();
            ...
            $image->move($destinationPath, $filename);
            $seller->passport_upload = $userFolder . '/' . $filename;
        }
        if (request()->hasFile('mbl_ll_bill_upload')) {
            $image = $request->file('mbl_ll_bill_upload');
            $filename = $image->getClientOriginalName();
            ...
            $image->move($destinationPath, $filename);
            $seller->mbl_ll_bill_upload = $userFolder . '/' . $filename;
        }
